...
@parser(description="Remove leading and trailing whitespace", tags=["string", "cleaning"])
def strip(column: pl.Expr) -> pl.Expr:
    """Leading and trailing whitespace removed."""
    return column.str.strip_chars()


@parser(description="Convert string to integer (i64)", tags=["conversion", "numeric"])
def to_int(column: pl.Expr) -> pl.Expr:
    """Cast to Int64, leaving unparsable values null."""
    return column.cast(pl.Int64, strict=False)


@parser(description="Convert string to float (f64)", tags=["conversion", "numeric"])
...
@parser(description="Convert string to lowercase", tags=["string", "normalization"])
def lower(column: pl.Expr) -> pl.Expr:
    """Lowercased."""
    return column.str.to_lowercase()


@parser(description="Convert string to uppercase", tags=["string", "normalization"])
